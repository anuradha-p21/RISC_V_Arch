/***********************************************************************
* FILENAME 		:StartDecoder.cpp
*
* DESCRIPTION 	:startdecoder methods are defined here.
*
...
void startdecoder::start_process()
{
    ...
    while (done) {
...

        {
            ...
            {
                msg = msg + " fetch (" + codeDesc[desc] + " " + " x1 = " + to_string(get_reg_val(rs1)) + " , " + to_string(get_reg_val(rd)) + ") ";
            }
            else
            {
        ...
        if (!fetch_Q.empty() && deco_dep == 0)
        {
            ...
            if (op_d.c_desc >= 0)
            {
                ...
                if (deco_sucess)  //sucessful, no depend
                {
                    deco_dep = 0;
                    decode_Q.push(op_d);
                    if (codeDesc[op_d.c_desc] == "lw" || codeDesc[op_d.c_desc] == "sw")
                    {
                        msg = msg + " decode (" + codeDesc[op_d.c_desc] + " " + " x1 = " + to_string(get_reg_val(op_d.rs1)) +  " , " + to_string(get_reg_val(op_d.rd)) + ") ";
                    }
                    else
                    {
        ...
        else if (deco_dep == 1)
        {
            ...
            if (deco_sucess) //dependencies cleared
            {
                ...
                deco_dep = 0;
                decode_Q.push(op_d);
                msg = msg + " decode (" + codeDesc[op_d.c_desc] + " " + to_string(get_reg_val(op_d.rs1)) + " , " + to_string(get_reg_val(op_d.rs2)) + " , " + to_string(get_reg_val(op_d.rd)) + ") ";
                //msg = msg + "decode (" + codeDesc[op_d.c_desc] + ") ";
            }
        ...
        if (!decode_Q.empty())
        {
            ...
            if (op_e.c_desc >= 0)
            {
                ...
                if (ex_sucess == 1) {
                    ...
                    {
                        msg = msg + " execute (" + codeDesc[op_e.c_desc] + " "  + to_string(get_reg_val(op_e.rs1)) + " , " + to_string(get_reg_val(op_e.rs2)) + " , " + to_string(get_reg_val(op_e.rd)) + ") ";
                    }
                }
                else if (ex_sucess == 3) {
                    msg = msg + " execute (" + codeDesc[op_e.c_desc] + ") " + "Flush the pipeline";
                    msg += "\n";
                    msg = msg + "clock :96 fetch (NOP) decode (NOP) execute (NOP) write-back (NOP) Program exit. ";
                    msg += "\n";
                    simu.push(msg);
                    end_process(0);
                    done = 0;
        ...
        if (!exe_Q.empty())
        {
            ...
            if (op_wb.c_desc >= 0) {
               ...
               if (codeDesc[op_wb.c_desc] == "lw" || codeDesc[op_wb.c_desc] == "sw")
                ...
                {
                    msg = msg + " write_back (" + codeDesc[op_wb.c_desc] + " "  + to_string(get_reg_val(op_wb.rs1)) + " , " + to_string(get_reg_val(op_wb.rs2)) + " , " + to_string(get_reg_val(op_wb.rd)) + ") ";
                }
            }
            else
            {
...
{
    if (e == 0) {
        log << "program executed sucessfully with return code: " << e << endl;
